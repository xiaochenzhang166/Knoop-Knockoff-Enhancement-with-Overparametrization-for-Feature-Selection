import numpy as np
import pandas as pd
from MSE_tools import get_MSE_list_for_BestKFeatures, get_bestKfeatures_indices_as_list
...
def load_pValues_as_a_list():
    ...
    :return:
    """
    p_value_array = np.genfromtxt('pVal.csv', delimiter=',', skip_header=1, dtype=str)
...
def save_everything_about_BestKFeatures(list_of_bestK_indices_lists, list_of_worst_pValues, MSE_list):
    ...
    output_file = 'Result_for_pKO.xlsx'  # 设置输出的 Excel 文件名
    df.to_excel(output_file, index=False, sheet_name="pKO")  # 将 DataFrame 写入 Excel，不保存索引列
