...
import pandas as pd
import pickle
from MSE_tools_with_linear_SVM import get_MSE_list_for_BestKFeatures, get_bestKfeatures_indices_as_list
...
def save_everything_about_BestKFeatures(list_of_bestK_indices_lists, list_of_worst_pValues, MSE_list):
    ...
    result = {'K': list(range(1, data_dim + 1)),
              ...
              'errorRateList': MSE_list}

    df = pd.DataFrame(result)

    output_file = 'Result_for_KnockoffBO- linear SVM edition.xlsx'  # 设置输出的 Excel 文件名
    df.to_excel(output_file, index=False)  # 将 DataFrame 写入 Excel，不保存索引列
