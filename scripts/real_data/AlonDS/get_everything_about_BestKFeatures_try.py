...
import pandas as pd
import pickle
from MSE_tools_try import get_MSE_list_for_BestKFeatures, get_bestKfeatures_indices_as_list
...
def save_everything_about_BestKFeatures(list_of_bestK_indices_lists, list_of_worst_pValues, errorRate_list):
    """
    ...
    result = {'K': list(range(1, data_dim + 1)),
              ...
              'ClassificationErrorRate': errorRate_list}

    df = pd.DataFrame(result)

    output_file = 'Result_for_KnockoffBO.xlsx'  # 设置输出的 Excel 文件名
    ...
    # 将对象保存到文件中
    with open('list_of_bestK_indices_lists.pkl', 'wb') as file:
        pickle.dump(list_of_bestK_indices_lists, file)
