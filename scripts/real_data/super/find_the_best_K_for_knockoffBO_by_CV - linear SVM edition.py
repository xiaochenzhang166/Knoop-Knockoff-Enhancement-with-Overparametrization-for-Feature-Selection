...
from get_everything_about_BestKFeatures_linear_SVM_edition import get_indices_from_the_best_feature_alone_to_all_features, get_MSE_list_for_BestKFeatures
from parameters import data_dim, knockoff_hier

def get_datasets_for_knockoffBO_CV():
    all_X_train, all_y_train, X_test, y_test = load_datasets_for_GeneralizedLinearRegression()
    print(all_y_train.shape)
    print(all_X_train.shape)
    X_train_part_1 = all_X_train[0:120, :]
    y_train_part_1 = all_y_train[0:120]
    X_train_part_2 = all_X_train[120:240, :]
    y_train_part_2 = all_y_train[120:240]
    X_train_part_3 = all_X_train[240:, :]
    y_train_part_3 = all_y_train[240:]
    # Cross-validation trial 1 - leave part5 as test set in this trail
    X_train_CV_1 = np.concatenate((X_train_part_1, X_train_part_2))
    y_train_CV_1 = np.concatenate((y_train_part_1, y_train_part_2))
    X_test_CV_1 = X_train_part_3
    y_test_CV_1 = y_train_part_3
    ...
    X_train_CV_2 = np.concatenate((X_train_part_2, X_train_part_3))
    y_train_CV_2 = np.concatenate((y_train_part_2, y_train_part_3))
    ...
    X_train_CV_3 = np.concatenate((X_train_part_1, X_train_part_3))
    y_train_CV_3 = np.concatenate((y_train_part_1, y_train_part_3))
    X_test_CV_3 = X_train_part_2
    y_test_CV_3 = y_train_part_2
    dataset_CV_3 = (X_train_CV_3, y_train_CV_3, X_test_CV_3, y_test_CV_3)


    return dataset_CV_1, dataset_CV_2, dataset_CV_3
...
def save_bestK_list(bestK_list, mse_list):
    ...
    output_file = 'Result_for_CV_KnockoffBO - linear SVM.xlsx'  # 设置输出的 Excel 文件名
